...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 18.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/axmon3y/axproxy/main/squid.conf
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/axmon3y/axproxy/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 7777 -j ACCEPT
    /sbin/iptables-save
